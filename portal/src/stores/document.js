...
export const useDocumentStore = defineStore('document', () => {
  ...
  async function fetchFiles(path = '') {
    ...
    try {
      ...
      currentFiles.value = response.data.items || response.data.data || []
    } catch (error) {
  ...
  async function searchDocs(keyword, category = '', mode = 'filename') {
    try {
      const params = {}
      ...
      const response = await fileBrowserApi.get('/search', { params })
  ...
  function getFileUrl(filename, path = '') {
    ...
    return `/api/filebrowser/file?path=${encodeURIComponent(filePath)}`
  }
